...
Paste a job ID into the **job ID** field and click **Download Previous Job Result** to fetch results from an earlier run without re-submitting. This also enables the download buttons above for that job.
...
All nodes follow the same pipeline:
...
3. The node fetches the download URL and triggers the file download immediately. Download is available only for the format selected when the job was originally submitted. This also enables the download buttons above for that job.
